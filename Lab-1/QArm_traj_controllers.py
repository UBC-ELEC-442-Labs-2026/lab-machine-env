...
class BaseQArmController:
    def __init__(self, waypoints):
        ...
        self.gamma = 0
        self.gripCmd = 0
        self.ledCmd = np.array([0, 1, 0], dtype=np.float64)
        
        self.startTime = 0
        self.last_time = 0
        self.animated_points = []
        
        # Will be initialized in setup_3d_plot
        self.fig = None
        self.ax = None
    ...
    def update_plot_elements(self, x, y, z):
        """Helper to update the shared visual elements"""
...
class CartesianJacobianController(BaseQArmController):
    def __init__(self, waypoints):
        ...
        # Add drop line timing parameters
    ...
    def _animation_update(self, frame, myArm):
        ...
        # Periodic static drop lines for depth perception
        ...
        # Jacobian Trajectory Calculations
        ...
        myArm.read_write_std(phiCMD=self.q_next, gprCMD=self.gripCmd, baseLED=self.ledCmd)
    ...
    def run(self):
        ...
        with QArm(hardware=0, readMode=0) as myArm:
            ...
            _, self.q_next = self.myArmUtilities.inverse_kinematics(start_pos, self.gamma, myArm.measJointPosition[0:4])
...
class JointSpaceController(BaseQArmController):
    ...
    def _animation_update(self, frame, myArm):
        ...
        # 1. Interpolate the joint angles based on current time
        current_phi_cmd = self.joint_spline(t)
        
        # 2. Command the arm
        myArm.read_write_std(phiCMD=current_phi_cmd, gprCMD=self.gripCmd, baseLED=self.ledCmd)
        
        # 3. Read actual joints to see where the physical arm actually went
        current_joints = myArm.measJointPosition[0:4]
        
        # 4. Forward Kinematics purely for plotting in our 3D space
    ...
    def run(self):
        ...
        with QArm(hardware=0, readMode=0) as myArm:
            ...
            for pt in self.position_anchors:
                _, q_target = self.myArmUtilities.inverse_kinematics(pt, self.gamma, myArm.measJointPosition[0:4])
                self.phi_targets.append(q_target)
            
            # Convert targets to a numpy array (N waypoints x 4 joints)
            self.phi_targets = np.array(self.phi_targets)
            
            # Create a Cubic Spline in JOINT SPACE
            # axis=0 ensures it interpolates along the waypoints for each joint independently
            self.joint_spline = CubicSpline(self.time_anchors, self.phi_targets, axis=0, bc_type='clamped')
                
            print("Moving to starting position (WP0)...")
            myArm.read_write_std(phiCMD=self.phi_targets[0], gprCMD=self.gripCmd, baseLED=self.ledCmd)
            time.sleep(2.0) 
